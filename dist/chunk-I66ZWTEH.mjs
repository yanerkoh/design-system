...
import { styled as styled4, Text as Text2 } from "@tamagui/core";
...
var getFontSizeVariable = (inSize, opts) => {
  ...
  if (!token)
    return inSize;
  const conf4 = getConfig();
  return conf4.fontsParsed[opts?.font || conf4.defaultFontToken]?.size[token];
};
var getFontSizeToken = (inSize, opts) => {
  if (typeof inSize == "number")
    return null;
  const relativeSize = opts?.relativeSize || 0, conf4 = getConfig(), fontSize = conf4.fontsParsed[opts?.font || conf4.defaultFontToken]?.size || // fallback to size tokens
  conf4.tokensParsed.size, size = (inSize === "$true" && !("$true" in fontSize) ? "$4" : inSize) ?? ("$true" in fontSize ? "$true" : "$4"), sizeTokens = Object.keys(fontSize);
...
import React2, { useEffect, useLayoutEffect } from "react";
var IS_REACT_19 = typeof React2.use < "u";
var isWeb = true;
...
var withStaticProperties = (component, staticProps) => {
  const next = (() => {
    if (component[Decorated]) {
      const _ = React3.forwardRef(
        (props, ref) => React3.createElement(component, { ...props, ref })
      );
...
function getVariableValue(v, group) {
  ...
  if (group) {
    const token = getConfig2().tokensParsed[group]?.[v];
    if (token)
...
var useThemeState = (props, isRoot = false, keys) => {
  const { disable } = props, parentId = useContext3(ThemeStateContext);
  if (!parentId && !isRoot)
  ...
  const id = useId(), subscribe3 = useCallback2(
    ...
    const needsUpdate = props.passThrough ? false : isRoot || props.name === "light" || props.name === "dark" || props.name === null ? true : HasRenderedOnce.get(keys) ? keys?.current?.size ? true : props.needsUpdate?.() : true, [rerender, next] = getNextState(
      local,
      props,
      propsKey,
      isRoot,
  ...
  const state = useSyncExternalStore(subscribe3, getSnapshot, getSnapshot);
  return useIsomorphicLayoutEffect(() => {
    ...
    process.env.NODE_ENV === "development" && props.debug === "verbose" && console.warn(` \xB7 useTheme(${id}) scheduleUpdate`, propsKey, states.get(id)?.name), scheduleUpdate(id), HadTheme.set(keys, true);
  }, [keys, propsKey]), state;
};
...
import { useRef as useRef2, useSyncExternalStore as useSyncExternalStore2 } from "react";
var mediaState = (
...
function useMedia(componentContext, debug) {
  const componentState = componentContext ? States.get(componentContext) : null, internalRef = useRef2(null);
...
var getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, groupContext, elementType, startedUnhydrated, debug) => {
  ...
  const { asChild } = props, { accept } = staticConfig, { noSkip, disableExpandShorthands, noExpand, styledContext } = styleProps, { webContainerType } = conf3.settings, parentVariants = parentStaticConfig?.variants;
  for (const keyOg in props) {
    ...
    if (!noExpand) {
      if (keyInit === "disabled" && valInit === true && (viewProps["aria-disabled"] = true, (elementType === "button" || elementType === "form" || elementType === "input" || elementType === "select" || elementType === "textarea") && (viewProps.disabled = true), !variants2?.disabled))
        continue;
      if (keyInit === "testID") {
  ...
  if (isReactNative)
    viewProps.tabIndex === 0 && (viewProps.accessible ?? (viewProps.accessible = true));
...
var useInsertEffectCompat = isWeb ? React5.useInsertionEffect || useIsomorphicLayoutEffect : () => {
};
...
import React7, { Children, cloneElement, forwardRef, isValidElement, useRef as useRef3 } from "react";
...
function ThemeDebug({
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    return /* @__PURE__ */ jsxs(Fragment, { children: [
      ...
      /* @__PURE__ */ jsx3("div", { style: { color: "red" }, children: themeState.id }),
      children
    ] });
  }
  return children;
...
var Theme = forwardRef(function(props, ref) {
  ...
  let finalChildren = props["disable-child-theme"] ? Children.map(
    props.children,
    (child) => passThrough ? child : cloneElement(child, { "data-disable-theme": true })
  ) : props.children;
  if (ref)
    try {
      React7.Children.only(finalChildren), finalChildren = cloneElement(finalChildren, { ref });
    } catch {
    }
...
function getThemedChildren(themeState, children, props, isRoot = false, stateRef, passThrough = false) {
  ...
  if (process.env.NODE_ENV === "development" && props.debug === "visualize" && (children = /* @__PURE__ */ jsx4(ThemeDebug, { themeState, themeProps: props, children })), !shouldRenderChildrenWithTheme)
    return children;
  children = /* @__PURE__ */ jsx4(ThemeStateContext.Provider, { value: themeState.id, children });
  ...
  themeState.name === "dark" || themeState.name === "light") && (state.hasEverThemed = "wrapped"), shallow && themeState.parentId) {
    ...
    children = Children.toArray(children).map((child) => isValidElement(child) ? passThrough ? child : cloneElement(
      child,
      void 0,
      /* @__PURE__ */ jsx4(Theme, { name: parentState.name, children: child.props.children })
    ) : child);
  }
  ...
  if (isWeb) {
    ...
    if (children = /* @__PURE__ */ jsx4(
      ...
      children = /* @__PURE__ */ jsx4("span", { style: baseStyle, className: className2, children });
    }
    return children;
  }
  return children;
...
function themeable(Component, staticConfig, optimize = false) {
  const withTheme = React8.forwardRef(function(props, ref) {
    ...
    const element = (
      ...
      /* @__PURE__ */ jsx5(Component, { ref, ...rest, "data-disable-theme": true })
    );
    ...
    if (compName && (filteredProps || (filteredProps = {}), filteredProps.componentName = compName), "debug" in props && (filteredProps || (filteredProps = {}), filteredProps.debug = props.debug), "theme" in props && (filteredProps || (filteredProps = {}), filteredProps.name = props.theme), "themeInverse" in props && (filteredProps || (filteredProps = {}), filteredProps.inverse = props.themeInverse), "themeReset" in props && (filteredProps || (filteredProps = {}), filteredProps.reset = themeReset), optimize && !filteredProps)
      return element;
    let contents = /* @__PURE__ */ jsx5(Theme, { "disable-child-theme": true, ...filteredProps, children: element });
    if (context) {
      const Provider = context.Provider, contextValue = React8.useContext(context);
      contents = /* @__PURE__ */ jsx5(Provider, { ...contextValue, ...overriddenContextProps, children: contents });
    }
    return contents;
  });
...
function getStyleTags(styles) {
  if (IS_REACT_19 && styles.length)
    return /* @__PURE__ */ jsx6(Fragment2, { children: styles.map((styleObject) => {
      const identifier = styleObject[StyleObjectIdentifier];
...
var useComponentState = (props, animationDriver, staticConfig, config) => {
  const isHydrated = useDidFinishSSR(), needsHydration = !useIsClientOnly(), useAnimations = animationDriver?.isStub ? void 0 : animationDriver?.useAnimations, { isHOC } = staticConfig, stateRef = useRef4(
  ...
  const states2 = useState(initialState), state = props.forceStyle ? { ...states2[0], [props.forceStyle]: true } : states2[0], setState = states2[1];
  ...
  if (!isHydrated)
    ...
    (isAnimatedAndHydrated && !animationDriver?.classNameAnimation || isDisabledManually || isClassNameDisabled) && (noClass = true, process.env.NODE_ENV === "development" && props.debug === "verbose" && log("avoiding className", {
...
import { Children as Children2, cloneElement as cloneElement2, forwardRef as forwardRef2, isValidElement as isValidElement2, version, memo } from "react";
import { Fragment as Fragment3, jsx as jsx7 } from "react/jsx-runtime";
var is19 = version.startsWith("19.");
var Slot = memo(
  forwardRef2(function(props, forwardedRef) {
...
var Slottable = ({ children }) => /* @__PURE__ */ jsx7(Fragment3, { children });
...
function createComponent(staticConfig) {
  ...
  const component = React9.forwardRef((propsIn, forwardedRef) => {
    var _a;
    const internalID = process.env.NODE_ENV === "development" ? React9.useId() : "";
    process.env.NODE_ENV === "development" && startVisualizer && (startVisualizer(), startVisualizer = void 0), process.env.NODE_ENV === "test" && propsIn["data-test-renders"] && ((_a = propsIn["data-test-renders"]).current ?? (_a.current = 0), propsIn["data-test-renders"].current += 1);
    const { context, isReactNative } = staticConfig, debugProp = propsIn.debug, styledContextValue = context ? React9.useContext(context) : void 0;
    ...
    process.env.NODE_ENV === "development" && isClient && React9.useEffect(() => {
    ...
    const componentContext = React9.useContext(ComponentContext), groupContextParent = React9.useContext(GroupContext), animationDriver = componentContext.animationDriver, useAnimations = animationDriver?.useAnimations, componentState = useComponentState(
      props,
...
`
    ...
    if (splitStyles?.resolvedContextVariants) {
      const contextForVariants = staticConfig.context || staticConfig.parentStaticConfig?.context;
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`
    ...
    props.containerType !== "normal" && (nonTamaguiProps.onLayout = composeEventHandlers(
      ...
      setElementProps,
      animatedRef
    )), viewProps.ref = stateRef.current.composedRef, process.env.NODE_ENV === "development" && !isReactNative && !isText && isWeb && !isHOC && React9.Children.toArray(props.children).forEach((item) => {
...
` && console.error(
    ...
    events && !isReactNative && Object.assign(viewProps, getWebEvents(events)), process.env.NODE_ENV === "development" && time2 && time2`events`, process.env.NODE_ENV === "development" && debugProp === "verbose" && log("events", { events, attachHover, attachPress }), hooks.useEvents?.(viewProps, events, splitStyles, setStateShallow, staticConfig);
    ...
    hooks.useChildren && (useChildrenResult = hooks.useChildren(elementType, content, viewProps)), process.env.NODE_ENV === "development" && time2 && time2`use-children`, useChildrenResult ? content = useChildrenResult : content = React9.createElement(elementType, viewProps, content);
    ...
    if (needsReset && !hasEverReset && (stateRef.current.hasEverResetPresence = true), (needsReset || hasEverReset) && ResetPresence && (content = /* @__PURE__ */ jsx8(ResetPresence, { disabled: !needsReset, children: content })), process.env.NODE_ENV === "development" && time2 && time2`create-element`, "focusWithinStyle" in propsIn && (content = /* @__PURE__ */ jsx8(
      ...
      debugProp && console.info("overriddenContextProps", overriddenContextProps), content = /* @__PURE__ */ jsx8(Provider, { __disableMergeDefaultValues: true, ...overriddenContextProps, children: content });
    }
    if (process.env.NODE_ENV === "development" && time2 && time2`context-override`, startedUnhydrated && splitStyles && (content = /* @__PURE__ */ jsxs2(Fragment4, { children: [
      content,
  ...
  let res = component;
  (process.env.TAMAGUI_FORCE_MEMO || staticConfig.memo) && (res = React9.memo(res)), res.staticConfig = staticConfig;
  ...
  function styleable(Component2, options) {
    let out = IS_REACT_19 && typeof Component2 == "function" && Component2.length === 1 || Component2.render?.length === 2 ? Component2 : React9.forwardRef(Component2);
    const extendedConfig = extendStyledConfig(options?.staticConfig);
    return out = options?.disableTheme ? out : themeable(out, extendedConfig, true), (extendedConfig.memo || process.env.TAMAGUI_MEMOIZE_STYLEABLE) && (out = React9.memo(out)), out.staticConfig = extendedConfig, out.styleable = styleable, out;
  }
...
function spacedChildren(props) {
  ...
  if (!ensureKeys && !(hasSpace || hasSeparator || isZStack))
    return children;
  const childrenList = areChildrenArray ? children : React9.Children.toArray(children);
  ...
  for (let [index, child] of childrenList.entries()) {
    ...
    if (!isEmpty && React9.isValidElement(child) && child.type?.shouldForwardSpace && (child = React9.cloneElement(child, {
      ...
      /* @__PURE__ */ jsx8(React9.Fragment, { children: isZStack ? /* @__PURE__ */ jsx8(AbsoluteFill, { children: child }) : child }, `${index}0t`)
    ...
    ), final.push(/* @__PURE__ */ jsx8(React9.Fragment, { children: separator }, `${index}03t`)), hasSpace && final.push(
      createSpacer({
        key: `_${index}01t`,
        direction,
        space,
...
function usePropsAndStyle(props, opts) {
  ...
  }), componentContext = React10.useContext(ComponentContext), groupContext = React10.useContext(GroupContext), { state, disabled, setStateShallow } = useComponentState(
    props,
...
var useGetThemedIcon = (props) => {
  ...
  return (el) => el && (React12.isValidElement(el) ? React12.cloneElement(el, {
    ...props,
    color,
    // @ts-expect-error
    ...el.props
  }) : React12.createElement(el, props));
};

// node_modules/@tamagui/stacks/dist/esm/Stacks.js
import { View, styled as styled2 } from "@tamagui/core";

// node_modules/@tamagui/stacks/dist/esm/getElevation.js
import { getVariableValue as getVariableValue2, isAndroid as isAndroid2, isVariable as isVariable3 } from "@tamagui/core";
var getElevation = (size, extras) => {
  if (!size)
    return;
  const { tokens } = extras, token = tokens.size[size], sizeNum = isVariable3(token) ? +token.val : size;
  return getSizedElevation(sizeNum, extras);
};
var getSizedElevation = (val, { theme, tokens }) => {
  let num = 0;
  if (val === true) {
    const val2 = getVariableValue2(tokens.size.true);
...
var YStack = styled2(View, {
  flexDirection: "column",
  variants
});
YStack.displayName = "YStack";
var XStack = styled2(View, {
  flexDirection: "row",
  variants
});
...
function wrapChildrenInText(TextComponent, propsIn, extraProps) {
  ...
  return color && (props.color = color), fontFamily && (props.fontFamily = fontFamily), fontSize && (props.fontSize = fontSize), fontWeight && (props.fontWeight = fontWeight), letterSpacing && (props.letterSpacing = letterSpacing), textAlign && (props.textAlign = textAlign), size && (props.size = size), fontStyle && (props.fontStyle = fontStyle), maxFontSizeMultiplier && (props.maxFontSizeMultiplier = maxFontSizeMultiplier), React14.Children.toArray(children).map((child, index) => typeof child == "string" ? (
    ...
    /* @__PURE__ */ jsx9(TextComponent, { ...props, ...textProps, children: child }, index)
  ) : child);
}
...
var ButtonIcon = (props) => {
  const { children, scaleIcon = 1 } = props, { size, color } = useContext6(ButtonContext), iconSize = (typeof size == "number" ? size * 0.5 : getFontSize(size)) * scaleIcon;
  return useGetThemedIcon({ size: iconSize, color })(children);
};
var ButtonComponent = ButtonFrame.styleable(
  function(props, ref) {
    const { props: buttonProps } = useButton(props);
    return /* @__PURE__ */ jsx10(ButtonFrame, { "data-disable-theme": true, ...buttonProps, ref });
  }
);
...
function useButton({ textProps, ...propsIn }, { Text: Text3 = Button2.Text } = { Text: Button2.Text }) {
  const isNested = useContext6(ButtonNestingContext), propsActive = useProps(propsIn, {
    noNormalize: true,
    noExpand: true
  }), {
    icon,
    ...
    ...restProps,
    children: /* @__PURE__ */ jsx10(ButtonNestingContext.Provider, { value: true, children: inner }),
...
var ButtonDXPlus = styled4(Button2, {
  name: "ButtonDXPlus",
  borderRadius: 8,
  display: "flex",
  alignItems: "center",
...
function Button({
  ...
  return /* @__PURE__ */ jsx11(
    ButtonDXPlus,
    {
      ...
      ...props,
      children: /* @__PURE__ */ jsx11(Text2, { fontWeight: 600, color: textColor, alignSelf: "center", children })
    }
  );
}
