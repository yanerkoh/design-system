...
const NAME = "ListItem", ListItemFrame = styled(ThemeableStack, {
  ...
  const props = useProps(propsIn, {
    ...
    ellipse,
    children
  }, size = props.size || "$true", iconSize = getFontSize(size) * scaleIcon, getThemedIcon = useGetThemedIcon({ size: iconSize, color }), [themedIcon, themedIconAfter] = [icon, iconAfter].map(getThemedIcon), sizeToken = getTokens().space[props.space] ?? iconSize, spaceSize = getVariableValue(sizeToken) * scaleSpace, contents = wrapChildrenInText(Text, textProps);
  return {
    props: {
      ...rest,
